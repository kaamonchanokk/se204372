...
<div class="block-1">
<br>
  <h2 style="position: relative;left:50px">ตรวจสอบผลการฝึกงาน</h2>
  <br>
  <table border = 1>
<tr> 
...
<?php 
  foreach($statuspelist as $statuspelist)
{
    echo"<tr>
    <td>$statuspelist->date_intpre</td>
    <td>$statuspelist->place_intpre</td>
    <td>$statuspelist->a_intpre</td>";
    echo '<td><a class="btn btn-primary btn-xs" href="data:application/pdf;base64,'.base64_encode($statuspelist->file_intpre).' " onclick="debugBase64(this.href)" />ดูเอกสาร</a></td>';
    if($statuspelist->status_intpre==="อนุมัติ")
    {
      echo '<td style="color:green">';
      echo $statuspelist->status_intpre;
      echo '</td>';
    }
    else if($statuspelist->status_intpre==="ไม่อนุมัติ")
    {
