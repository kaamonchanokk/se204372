  ...
  foreach($documentList as $documentList)
{
    ...
    ";
    echo '<td><a class="btn btn-primary btn-xs" href="data:application/pdf;base64,'.base64_encode($documentList->file_doc).' "  onclick="debugBase64(this.href)" />ดาวน์โหลด</a></td>';
    ;?>
    <td><a class="btn btn-warning btn-xs" href=?controller=document&action=updateForm&<?php echo "id_doc=$documentList->id_doc";?>>📝</a></td>
     
     <td><form method="POST" action="?controller=document&action=delete">
      <input name="_method" type="hidden" value="DELETE">
      <input name="id_doc" type="hidden" value="<?php echo "$documentList->id_doc";?>">
      <button type="button" class="btn btn-xs btn-danger btn-flat show_confirm" data-toggle="tooltip" test="<?php echo "$documentList->name_doc";?>" title='Delete'>🗑️</button>
      </form></td>
    </tr>
    <?php
}
echo "</table>";
?>
<a class="btn btn-primary btn-xs" style="float:right;position: relative;right:200px" href=?controller=document&action=newdocument>เพิ่มข้อมูล</a><br>
<br>
<br>
<br>
<br>
...
<script type="text/javascript">
     ...
     $('.show_confirm').click(function(event) {
          ...
          Swal.fire({
          title: 'แน่ใจว่าคุณต้องการเอกสารนี้?',
          text: ""+test,
          icon: 'warning',
